...
function encode(str, n) {
  ...
  const key = [...n.toString()].map(el => Number(el));
  return [...str].map((el, i) => {
    if (i <= key.length - 1) return arr_en.indexOf(el) + 1 + key[i];
    else if (i > key.length - 1)
      return arr_en.indexOf(el) + 1 + key[i - key.length * Math.trunc(i / key.length)];
  });
}
